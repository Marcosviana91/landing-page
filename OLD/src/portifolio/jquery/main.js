$(document).ready(function () {
  {
    const efeitosBasicoBtnGroup = document.querySelectorAll(
      '#efeitos #basicos button'
    )
    const efeitosFadeBtnGroup = document.querySelectorAll(
      '#efeitos #fade button'
    )
    const input_fade = document.querySelector('#efeitos #fade input')
    const efeitosSlideBtnGroup = document.querySelectorAll(
      '#efeitos #slide button'
    )
    ...
    $(input_fade).change(function (e) {
      e.preventDefault()
      efeitosFadeBtnGroup[3].firstElementChild.innerHTML =
        input_fade.value / 100
    })
  ...
  }
  {
    ...
    $.each(
      $('section#seletores div#interagir *'),
    ...
    elemento1_Set.forEach(function (indexInArray, valueOfElement) {
      $('#elemento1').append(
        `<option value="${valueOfElement}">${valueOfElement}</option>`
      )
    })
    ...
    function preFiltrar() {
      ...
      if (elemento1 && tipo_filtro) {
        var lista_elemento_alvo = $(
          `#seletores #interagir ${elemento1} ${tipo_filtro} *`
        )
        $.each(lista_elemento_alvo, function (indexInArray, valueOfElement) {
          elemento_alvo_Set.add(valueOfElement.localName)
        })
        document.getElementById('elemento-alvo').innerHTML =
          '<option value="" selected disabled>Elemento alvo</option>'
        elemento_alvo_Set.forEach(function (indexInArray, valueOfElement) {
          $('#elemento-alvo').append(
            `<option value="${valueOfElement}">${valueOfElement}</option>`
          )
        })
      }
    ...
    function filtrar() {
      ...
      if (elemento1 && tipo_filtro && elemento_alvo) {
        var lista_elementos_filtrados = $(
          `#seletores #interagir ${elemento1} ${tipo_filtro} ${elemento_alvo}`
        )
        $.each(
          lista_elementos_filtrados,
          function (indexInArray, valueOfElement) {
            $(valueOfElement).css({
